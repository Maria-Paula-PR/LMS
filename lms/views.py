...
def panel_cursos(request):
    """Panel de cursos para estudiantes con estado de inscripción"""
    ...
    available_courses = []
    for course in courses.exclude(id__in=enrolled_course_ids):
    ...
    return render(request, 'usuarios/panel_cursos.html', {
...
@login_required
def curso_contenido(request, course_id):
    """Mostrar contenido del curso solo para inscritos"""
    course = get_object_or_404(Course, id=course_id, status='published')
    # Verificar que el usuario esté inscrito en el curso
    is_enrolled = Enrollment.objects.filter(student=request.user, course=course).exists()
    if not is_enrolled:
        messages.error(request, 'Debes inscribirte al curso para ver su contenido.')
        return redirect('panel_cursos')

    lessons = course.lessons.filter(is_published=True).order_by('order')
    return render(request, 'usuarios/curso_contenido.html', {
        'course': course,
        'lessons': lessons
    })
